  ...
  try {
    ...
    window.WemoOwnerContent = {
      cards(page) {
        ...
        return (state.listings || []).filter(l=>l.active && l.end>=today && (page==='events'?l.type==='Event':l.type!=='Event')).map(l=>`<article class="place-card"><a class="place-card__image" href="business.html#public"><img src="${escape(validImage(l.image))}" alt="${escape(l.title)}"><span class="place-tier">${escape(window.WemoI18n.lang==='ka'?({Offer:'შეთავაზება',Event:'ღონისძიება',Experience:'გამოცდილება'}[l.type]||l.type):l.type)}</span></a><div class="place-card__body"><span class="tag">${escape(b.name)}</span><a href="business.html#public"><h3>${escape(l.title)}</h3></a><p class="meta">${escape(l.start)} – ${escape(l.end)}</p><p class="rating">₾${Number(l.price).toLocaleString()} / ${window.WemoI18n.lang==='ka'?'სტუმარი':'guest'}</p></div></article>`).join('');
      }
    };
